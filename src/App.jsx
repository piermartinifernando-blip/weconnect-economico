...
const CLIENTES_STAT = {
  ...
  porCiudad:[
    {ciudad:"Almirante Brown",    habilitados:1867, total:3128, deuda:62.43, deudaVenc:55.91},
    {ciudad:"Capitan Sarmiento",  habilitados:1300, total:1740, deuda:12.51, deudaVenc:12.24},
    {ciudad:"Ministro Rivadavia", habilitados:348,  total:401,  deuda:4.39,  deudaVenc:0.91 },
    {ciudad:"Glew",               habilitados:324,  total:395,  deuda:5.19,  deudaVenc:1.42 },
    {ciudad:"Longchamps",         habilitados:125,  total:152,  deuda:1.40,  deudaVenc:0.35 },
    {ciudad:"Florencio Varela",   habilitados:58,   total:93,   deuda:1.55,  deudaVenc:1.11 },
    {ciudad:"Burzaco",            habilitados:42,   total:51,   deuda:0.55,  deudaVenc:0.13 },
  ],
  mediosPago:[
...
const CHURN={
  ...
  cohortes:[
    {cohorte:"Ago-Dic 24 (+18m)",pct:39.3,inactivos:476,color:C.red  },
    {cohorte:"Q1-Q2 25 (9-15m)", pct:37.7,inactivos:583,color:C.red  },
    {cohorte:"Q3 25 (6-9m)",     pct:37.9,inactivos:307,color:C.amber},
    {cohorte:"Q4 25 (3-6m)",     pct:25.1,inactivos:214,color:C.amber},
    {cohorte:"2026 (0-3m)",      pct:12.2,inactivos:72, color:C.green},
  ],
  causas:[
...
const NETO_DATA=[
  ...
  {mes:"Mar 26",cobrado:94.03, egresos:183.34,neto:-89.31,altas:138,bajas:9 },
];
...
const KPI=({label,value,sub,ok=true,accent=C.cyan})=>(
  <div style={{background:C.card,border:`1px solid ${C.b1}`,borderRadius:12,padding:"18px 20px",position:"relative",overflow:"hidden"}}>
...
export default function App(){
  ...
  const motor=useMemo(()=>{
    ...
    const pl=egresos.map(e=>{
      ...
      return{mes:e.mes,cobrado:c.cobrado,opex:e.opex,capex:e.capex,resultado:+(c.cobrado-e.opex-e.capex).toFixed(1)};
    });
    const ARPU=27425,COSTO_V=7630,churnR=0.025;
    ...
    let cliSin=CLIENTES_STAT.habilitados,cliCon=CLIENTES_STAT.habilitados;
    const proyeccion=mesesBE.map(mes=>{
      cliSin=Math.round(cliSin*(1-churnR)+195);
      cliCon=Math.round(cliCon*(1-churnR)+420);
      return{mes,sinPlan:+((cliSin*ARPU/1e6)-ultEgr.opex).toFixed(1),conPlan:+((cliCon*ARPU/1e6)-ultEgr.opex).toFixed(1),cliSin,cliCon};
    });
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:C.sans,color:C.txt,display:"flex"}}>
      ...
      <div style={{position:"fixed",top:0,left:0,bottom:0,width:205,background:C.surf,borderRight:`1px solid ${C.b1}`,display:"flex",flexDirection:"column",zIndex:50}}>
        <div style={{padding:"18px 16px",borderBottom:`1px solid ${C.b1}`}}>
          <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:10}}>
            <div style={{width:32,height:32,borderRadius:8,background:`linear-gradient(135deg,${C.cyan},${C.green})`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,color:"#0f1117",fontSize:14}}>W</div>
            <div>
              <p style={{fontSize:13,fontWeight:800,color:C.txt}}>WeConnect</p>
        ...
        <div style={{padding:"12px 16px",borderTop:`1px solid ${C.b1}`}}>
          ...
          <p style={{fontSize:9,color:C.muted}}>4.064 clientes activos</p>
          <p style={{fontSize:9,color:C.muted}}>Deuda total: $88M</p>
        </div>
      </div>
      ...
      <div style={{marginLeft:205,padding:"22px 26px",flex:1}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
          <div>
            <h1 style={{fontSize:18,fontWeight:800,letterSpacing:"-0.02em",color:C.txt}}>
          ...
          {motor&&(
            <div style={{display:"flex",gap:8}}>
              <div style={{background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.25)",borderRadius:8,padding:"7px 14px"}}>
              ...
              <div style={{background:"rgba(251,191,36,0.1)",border:"1px solid rgba(251,191,36,0.25)",borderRadius:8,padding:"7px 14px"}}>
              ...
              <div style={{background:"rgba(56,189,248,0.1)",border:"1px solid rgba(56,189,248,0.25)",borderRadius:8,padding:"7px 14px"}}>
        ...
        {tab==="negocio"&&motor&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
              <KPI label="Cobrado Mar 26 (parcial)" value={fM(94.03)} sub="Al 23/3 · en máximos" accent={C.cyan} ok={true}/>
              <KPI label="Cobrado Feb 26" value={fM(95.21)} sub="▲ vs Ene 26 ($95.98M)" accent={C.green} ok={true}/>
              <KPI label="Máximo histórico" value={fM(95.98)} sub="Enero 2026" accent={C.purple} ok={true}/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:14}}>
              <Card title="Evolución de cobranza — histórico real ($M ARS)">
                ...
                <div style={{marginTop:10,padding:"8px 12px",background:"rgba(52,211,153,0.08)",border:"1px solid rgba(52,211,153,0.2)",borderRadius:8}}>
                  <p style={{fontSize:11,color:C.green,fontWeight:600}}>✦ Crecimiento: $5.66M (ago 24) → $94M (mar 26) · +1.562%</p>
              ...
              <Card title="Canales de cobro — Mar 26">
                ...
                <div style={{marginTop:10,padding:"8px 10px",background:"rgba(251,191,36,0.08)",borderRadius:8,border:"1px solid rgba(251,191,36,0.2)"}}>
        ...
        {tab==="clientes"&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
              ...
              <KPI label="Habilitados"   value="4.064" sub="68.2% del padrón"     accent={C.green}  ok={true}/>
            ...
            <Card title="Curva neto: Ingresos − Egresos · con altas y bajas de clientes">
              <ResponsiveContainer width="100%" height={260}>
                <ComposedChart data={NETO_DATA}>
                  <CartesianGrid strokeDasharray="3 3" stroke={C.b1}/>
                  <XAxis dataKey="mes" stroke={C.muted} tick={{fontSize:11,fill:C.muted}}/>
                  <YAxis yAxisId="left"  stroke={C.muted} tick={{fontSize:10,fill:C.muted}} tickFormatter={v=>`$${v}M`}/>
                  <YAxis yAxisId="right" orientation="right" stroke={C.muted} tick={{fontSize:10,fill:C.muted}}/>
                  <Tooltip content={<Tip/>}/>
                  <Legend formatter={v=><span style={{color:C.sub,fontSize:11}}>{v}</span>}/>
                  <ReferenceLine yAxisId="left" y={0} stroke={C.cyan} strokeDasharray="4 3" strokeWidth={1.5}/>
                  <Area yAxisId="left"  type="monotone" dataKey="neto"   name="Neto ($M)" stroke={C.red}   fill="rgba(248,113,113,0.15)" strokeWidth={2.5}/>
                  <Bar  yAxisId="right" dataKey="altas"  name="Altas"    fill={C.green}   opacity={0.7} radius={[3,3,0,0]}/>
                  <Bar  yAxisId="right" dataKey="bajas"  name="Bajas"    fill={C.amber}   opacity={0.7} radius={[3,3,0,0]}/>
                </ComposedChart>
              </ResponsiveContainer>
              <div style={{marginTop:10,padding:"8px 12px",background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.2)",borderRadius:8}}>
                <p style={{fontSize:11,color:C.red,fontWeight:600}}>Déficit promedio $-74M/mes · se cierra cuando altas escalen y egresos se estabilicen</p>
                <p style={{fontSize:10,color:C.sub,marginTop:2}}>Las altas de mar 26 (138) reflejan mes parcial al 23/3</p>
              </div>
            </Card>

            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:14}}>
              <Card title="Altas brutas y estado por mes — últimos 6 meses">
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={clientes} barGap={4}>
                    ...
                    <Bar dataKey="habilitados" name="Quedan activos" fill={C.cyan}  radius={[3,3,0,0]}/>
                    <Bar dataKey="bajas"       name="Sin servicio"   fill={C.red}   radius={[3,3,0,0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </Card>
              <Card title="Distribución por plan (habilitados)">
                  ...
                  <div key={i} style={{marginBottom:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      ...
                      <div style={{display:"flex",gap:8}}>
                        ...
                        <span style={{fontSize:11,color:p.color,fontFamily:C.mono,fontWeight:700}}>{((p.cantidad/4064)*100).toFixed(1)}%</span>
                      </div>
                    </div>
                    <div style={{height:4,background:C.b1,borderRadius:4,overflow:"hidden"}}>
                ...
                <div style={{marginTop:10,padding:"8px 10px",background:"rgba(56,189,248,0.08)",borderRadius:8,border:"1px solid rgba(56,189,248,0.2)"}}>
            ...
            <Card title="Distribución por ciudad — habilitados reales vs total padrón">
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                {CLIENTES_STAT.porCiudad.map((c,i)=>{
                  const moraPct=((c.deudaVenc/c.deuda)*100).toFixed(0);
                  const actPct=((c.habilitados/c.total)*100).toFixed(0);
                  ...
                  return(
                    <div key={i} style={{background:C.surf,borderRadius:10,padding:"12px 14px",border:`1px solid ${C.b1}`}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                        <p style={{fontSize:10,fontWeight:600,color:C.txt}}>{c.ciudad}</p>
                        <span style={{background:`${col}20`,color:col,padding:"1px 7px",borderRadius:20,fontSize:9,fontWeight:700}}>{moraPct}% mora</span>
                      </div>
                      <p style={{fontSize:18,fontFamily:C.mono,fontWeight:700,color:C.cyan}}>{c.habilitados.toLocaleString("es-AR")}</p>
                      <p style={{fontSize:9,color:C.muted,marginTop:2}}>de {c.total.toLocaleString("es-AR")} · {actPct}% activos</p>
                      <div style={{height:3,background:C.b1,borderRadius:4,marginTop:8,overflow:"hidden"}}>
                        <div style={{width:`${actPct}%`,height:"100%",background:C.cyan,borderRadius:4}}/>
                      </div>
                      <p style={{fontSize:9,color:C.muted,marginTop:4}}>Deuda venc: {fM(c.deudaVenc)}</p>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:14}}>
              ...
              <Card title="Causa raíz — medios de pago">
                ...
                <div style={{marginTop:10,padding:"10px 12px",background:"rgba(248,113,113,0.08)",borderRadius:8,border:"1px solid rgba(248,113,113,0.2)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
              {[{label:"Win-back 20%",pct:0.2,color:C.sub},{label:"Win-back 30%",pct:0.3,color:C.amber},{label:"Win-back 40%",pct:0.4,color:C.green}].map((e,i)=>{
                ...
                return(
                  <div key={i} style={{background:C.surf,border:`1px solid ${C.b1}`,borderRadius:10,padding:"16px 18px",textAlign:"center"}}>
                    ...
                    <div style={{background:`${e.color}18`,borderRadius:6,padding:"6px",marginTop:8}}>
            ...
            <Card title="Proyección resultado mensual — Con plan vs Sin plan ($M ARS)">
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={motor.proyeccion}>
                  ...
                  <YAxis stroke={C.muted} tick={{fontSize:10,fill:C.muted}} tickFormatter={v=>`$${v}M`}/>
                  <Tooltip content={<Tip/>}/>
                  <ReferenceLine y={0} stroke={C.cyan} strokeWidth={1.5} strokeDasharray="4 3" label={{value:"Break-even",fill:C.cyan,fontSize:11,position:"right"}}/>
                  <Legend formatter={v=><span style={{color:C.sub,fontSize:11}}>{v}</span>}/>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                ...
                <tbody>
                  {motor.proyeccion.map((r,i)=>(
                    <tr key={i} style={{borderTop:`1px solid ${C.b1}`,background:r.conPlan>=0&&motor.proyeccion[i-1]?.conPlan<0?"rgba(52,211,153,0.06)":"transparent"}}>
        ...
        {tab==="churn"&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
              <KPI label="Churn acumulado"    value="31.6%" sub="1.896 de 5.960 inactivos" accent={C.red}   ok={false}/>
              <KPI label="Tasa mensual prom." value="2.9%"  sub="~118 clientes/mes"         accent={C.red}   ok={false}/>
              <KPI label="Churn anual impl."  value="~38%"  sub="Cohortes maduras convergen" accent={C.amber} ok={false}/>
              ...
              <Card title="Tasa de churn mensual (%)">
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={CHURN.mensual}>
                    ...
                    <Tooltip content={<Tip/>}/>
                    <ReferenceLine y={2.9} stroke={C.amber} strokeDasharray="4 3" label={{value:"Prom 2.9%",fill:C.amber,fontSize:10}}/>
                    <Line type="monotone" dataKey="pct" name="Churn %" stroke={C.red} strokeWidth={2.5} dot={false}/>
                  </LineChart>
                </ResponsiveContainer>
              </Card>
              <Card title="Churn acumulado por cohorte — datos reales verificados">
                ...
                <div style={{marginTop:10,padding:"8px 10px",background:"rgba(251,191,36,0.08)",borderRadius:8,border:"1px solid rgba(251,191,36,0.2)"}}>
                  <p style={{fontSize:10,color:C.amber,fontWeight:600}}>⚠ Churn estructural ~38% en cohortes maduras · requiere acción sistémica</p>
                  <p style={{fontSize:10,color:C.green,marginTop:4}}>✦ 2026 baja al 12.2% → calidad de captación mejora</p>
                </div>
              </Card>
            </div>
            <Card title="Causas del churn y palancas de retención">
              ...
              <div style={{marginTop:12,padding:"10px 14px",background:"rgba(52,211,153,0.08)",borderRadius:8,border:"1px solid rgba(52,211,153,0.2)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
